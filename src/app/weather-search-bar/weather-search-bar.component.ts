...
@Component({
  ...
  template: `<form>
  <input type="text" placeholder="Search city" [(ngModel)]="search" name="search" />
  <img
    src="../../assets/Images/inspect/weather/Web/01_Home/Group 2/icon_search_white.png"
    class="search_icon" (click)="getWeatherData()"
  />
</form>`,
...
export class WeatherSearchBarComponent implements OnInit{
  ...
  getWeatherData(){
    fetch('https://api.openweathermap.org/data/2.5/weather?q='+this.search+'&appid=cfa27c46b21352fbc56c8e592ffb6eff').then(response=>response.json())
